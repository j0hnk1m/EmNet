...
def main():
	...
	nClass = 7
	X1, Xtarget1, Xinfo1 = emo.load_emodb_ftr_ext_new(modeFsel=modeFsel)
	emo.summary_dbinfo(Xinfo1)
		
	nFile = (X1.shape)[0]
	dimFeatIn = (X1.shape)[2]
	dimTime = TimeForced
	if modeTD:
	...
	nlogo = logo.get_n_splits(X1, Xtarget1, groups)
	z=logo.split(X1, Xtarget1, groups)
	...
	if strModel=='svm':
		...
		XtargetNetout = np_utils.to_categorical(Xtarget1, nClass)
		...
		for h in range(0, 5):
			...
			# normalization
			XnormSm1, XnormStatic1 = emo.feature_normalization(X1, Xinfo1, idxLOSO=h, modeSpkrdep=modeSpkrdep)
			...
			if modeTD:
				XnormSm = emo.get_tdfeature(XnormSm, Xinfo1)
				if modeFsel==0:
					dimUVChange.extend([34, 35, 36])
				elif modeFsel==2:   # non-TD dim = 24
					dimUVChange.extend([38, 39, 40])

			XnormSm1 = emo.smooth(XnormSm1, a=0.9)    # smoothing
			XnormSm = emo.set_uvftrval(XnormSm, Xinfo1, dimUVChange)
			XnormSmTN = emo.reshape_feature_fixedtime(XnormSm, np.array(Xinfo1.nFrame), TimeForced)  # zero-padding &
			#  truncating
