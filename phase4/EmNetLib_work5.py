...
def load_emodb_ftr_ext_new(modeFsel, fName='EMO_frameVectorExtended_pyAudioAnalysis.json'):
	...
	for n in range(0, nFile):
		...
		idVoiced = [i for i in range(nFrame) if (ftr[i, iPv] > 0.2) & (ftr[i, iLogEnergy] > 37.0)]  # for pyAudioAnalysis
...
def run_svm(training_data, test_data, c, k, g, r):
	np.random.seed(r)
	
	clf = svm.SVC(C=c, kernel=k, gamma=g, random_state=r)
